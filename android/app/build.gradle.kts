...
android {
    ...
    // ── Product flavors ───────────────────────────────────────────────────────
    // Two APK variants from a single module:
    //   mobile  – phone / tablet, bottom-nav UI, LAUNCHER intent-filter.
    //   tv      – Android TV / Google TV, side-rail UI, LEANBACK_LAUNCHER intent-filter,
    //             tv_banner drawable, sensorLandscape orientation.
    //
    // Build commands:
    //   ./gradlew assembleMobileRelease   → VortX-mobile-release.apk
    //   ./gradlew assembleTvRelease       → VortX-tv-release.apk
    flavorDimensions += "target"
    productFlavors {
        create("mobile") {
            dimension = "target"
            applicationIdSuffix = ""          // com.stremiox.android
            versionNameSuffix = "-mobile"
        }
        create("tv") {
            dimension = "target"
            applicationIdSuffix = ".tv"       // com.stremiox.android.tv
